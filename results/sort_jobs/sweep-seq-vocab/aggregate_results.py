...
CSVs are written into <run_dir>/data/ and plots into <run_dir>/graphs/.
"""
...
def find_csv_files():
    pattern = os.path.join(DATA_DIR, "seq_*_maxv_*.csv")
...
def make_pivot_and_plot(rdf, value_col, cmap='viridis', fmt=':.3f', to_percent=False, vmin=None, vmax=None):
    ...
    mat = rdf.pivot(index='seq_length', columns='vocab_size', values=value_col).reindex(index=seqs[::-1], columns=vocabs)
    ...
    out_csv = os.path.join(DATA_DIR, f'{value_col}_matrix.csv')
    ...
    plt.tight_layout()
    out_png = os.path.join(GRAPHS_DIR, f'{value_col}_heatmap.png')
...
def plot_final_accuracy_lines(rdf):
    ...
    plt.grid(True)
    plt.tight_layout()
    out_png = os.path.join(GRAPHS_DIR, 'token_acc_vs_seq_by_vocab.png')
    plt.savefig(out_png, dpi=150)
    plt.close()
    print(f'Wrote {out_png}')
    ...
    plt.grid(True)
    plt.tight_layout()
    out_png = os.path.join(GRAPHS_DIR, 'exact_acc_vs_seq_by_vocab.png')
    plt.savefig(out_png, dpi=150)
    plt.close()
    print(f'Wrote {out_png}')
...
def main():
    ...
    combined_csv = os.path.join(DATA_DIR, 'sweep_combined_long.csv')
