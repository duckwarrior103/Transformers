"""Plot palindrome elm_comparison sweep — deltanet/gated_deltanet/linear_attention vs GEAD ELM sizes.
...
def _add_comparison_lines(ax, models, get_x, get_y, ycol, scale):
    """Plot lines for a list of models; returns True if anything was plotted."""
    plotted = False
    for model in models:
        x, y = get_x(model), get_y(model, ycol, scale)
...
def plot_comparison(all_model_finals, graphs_dir, ycol, ylabel, title, filename, scale=1.0):
    ...
    def get_x(model):
        ...
        return rdf["seq_length"].tolist() if rdf is not None else None

    def get_y(model, col, sc):
        rdf = all_model_finals.get(model)
        if rdf is None or col not in rdf.columns or rdf[col].isna().all():
            return None
        return (rdf[col] * sc).tolist()

    _add_comparison_lines(ax, MODELS, get_x, get_y, ycol, scale)
...
def plot_test_comparison(tdf, graphs_dir, ycol, ylabel, title, filename, scale=1.0):
    ...
    def get_x(model):
        ...
        return mdf["seq_length"].tolist() if not mdf.empty else None

    def get_y(model, col, sc):
        mdf = tdf[tdf["model_type"] == model].sort_values("seq_length")
        if mdf.empty or col not in mdf.columns:
            return None
        return (mdf[col] * sc).tolist()
