"""Plot palindrome elm_comparison LR scan results.
...
def load_model_steps_lr(run_dir, model, lr):
    """Load per-step CSVs for a given model and LR. Returns {seq: df}."""
    lr_str = f"{lr:g}"
    all_data = {}
...
def compute_peak_acc(all_data):
    """For each seq_length, return peak token_acc and exact_acc over all steps."""
    records = []
...
def select_best_lr(run_dir, model, lrs):
    """Select the LR with the highest mean peak token_acc across seq lengths."""
    best_lr = None
    best_score = -1
    for lr in lrs:
...
def plot_lrscan_summary(run_dir, graphs_dir, lrs):
    """Plot avg peak token_acc vs LR for each model."""
...
def main():
    ...
    # Detect available LRs from directory structure
    available_lrs = set()
    for model in MODELS:
    ...
    # 1. LR scan summary plot
    print("\n--- LR scan summary ---")
    plot_lrscan_summary(run_dir, graphs_dir, lrs)

    # 2. Select best LR per model
    best_lrs = {}
    for model in MODELS:
    ...
    # 3. Load peak accuracy at best LR for each model
    all_model_peaks = {}
    all_model_steps = {}
    ...
    # 4. Peak accuracy comparison plots
    if all_model_peaks:
    ...
    # 5. Convergence plots at best LR
    if all_model_steps:
